...
# 3. Input / Output
LOCAL_AUDIO_FILE = "soundtest2/เลือกหุ้น_พร้อมลุย_ตลาดบ่าย_06112568.mp3"
...
def extract_financial_entities_with_llm(raw_text):
    ...
    คืนเป็น list ของ dict; ถ้า parse JSON ไม่ได้ให้คืน [] แทน
    """
    print("🧠 Running Stock NER Agent (LLM)...")
    ...
    system_prompt = (
        ...
        "ห้ามใช้ single quote, ห้ามใส่ comment, ห้ามมี comma เกินท้าย array\n"
        "ตัวอย่างรูปแบบที่ถูกต้อง:\n"
        "[{\"mention\": \"ไทยคม\", \"type\": \"stock_th\", \"note\": \"satellite\"}]"
    )
...
"""
    ...
    try:
        ...
        # DEBUG ถ้าอยากดู raw output ให้ uncomment
        # print("RAW NER OUTPUT:\n", content)

        # ดึงเฉพาะส่วนที่เป็น array [ ... ]
        start = content.find('[')
        end = content.rfind(']')
        if start == -1 or end == -1 or end <= start:
            print("⚠️ ไม่เจอ JSON array ใน NER output, return []")
            return []
        ...
        try:
            ...
            print(f"❌ JSON parse error from NER: {e}")
            print("⚠️ Raw JSON candidate (ตัดให้สั้น):", json_str[:500])
            return []
        ...
        print(f"❌ Stock NER LLM Error: {e}")
        return []

# ---------------- AGENT TOOLS: DuckDuckGo + yfinance ----------------
...
def enrich_entities_with_tools(entities):
    ...
    """
    print("🔍 Resolving tickers with tools...")
    enriched = []
    for e in entities:
        e_type = e.get("type")
