...
class AgentState(TypedDict):
    ...
    channel_name: str
    chunk_size: int
    current_text: str       # Text ล่าสุดที่กำลังแก้
    feedback_msg: str       # Feedback จากรอบก่อน
    iteration_count: int    # นับรอบ
    verified_mappings: List[str]
    unknown_tickers: List[str]
    correction_suggestions: List[str] # รายการคำแนะนำการแก้ไขจาก Fact-Checker
    chunks_with_issues: List[int] # [NEW] เก็บ index ของ chunk ที่มีปัญหา
    verified_entities_cache: Dict[str, str] # Cache สำหรับเก็บ Entity ที่ตรวจสอบแล้ว (ตัวอย่าง: {"macisen seven": "Magnificent Seven"})
    ...
    # --- Original Fields ---
    quality_score: int      # คะแนนคุณภาพภาษา (1-10)
    final_summary: str      # ผลสรุปสุดท้ายในรูปแบบ Markdown
    data_managers: Dict[str, Any] # Dictionary สำหรับเก็บ instances ของ Manager ต่างๆ

# === AGENT GRAPH NODES ===

def validate_ticker_hallucination(corrected_text: str, raw_text: str, kb_data: dict) -> str:
    """
    Remove hallucinated tickers that don't exist in RAW or Knowledge Base.
    
    Logic:
    1. Flatten nested KB structure {SECTOR: {TICKER.BK: [variations]}}
    2. Extract all ticker patterns (2-5 uppercase letters)
    3. For each ticker, check if valid:
       - Must exist in KB (with or without .BK suffix)
       - At least one Thai variation must appear in RAW text
    4. If invalid, replace with [หุ้น] placeholder
    
    Args:
        corrected_text: Text after LLM correction
        raw_text: Original RAW transcript
        kb_data: Knowledge base with nested structure
        
    Returns:
        Cleaned text with hallucinated tickers removed
    """
    import re
    
    # Step 1: Flatten KB structure
    # KB is {SECTOR: {TICKER.BK: [variations]}} → flatten to {TICKER: [variations]}
    ticker_map = {}
    for sector, tickers in kb_data.items():
        if isinstance(tickers, dict):  # Skip non-sector keys
            for ticker_full, variations in tickers.items():
                # ticker_full is like "AMATA.BK" → extract "AMATA"
                ticker_base = ticker_full.replace('.BK', '').upper()
                ticker_map[ticker_base] = variations
    
    # Extract all ticker-like patterns (2-5 uppercase letters)
    ticker_pattern = r'\b[A-Z]{2,5}\b'
    tickers_found = re.findall(ticker_pattern, corrected_text)
    
    if not tickers_found:
        print("   ℹ️  No tickers found in text")
        return corrected_text
    
    print(f"   🔍 Found {len(set(tickers_found))} unique ticker(s): {', '.join(sorted(set(tickers_found)))}")
    print(f"   📚 KB contains {len(ticker_map)} tickers")
    
    # Common non-ticker words to skip
    SKIP_WORDS = {'SET', 'CLEAN', 'RAW', 'HIGH', 'LOW', 'GAP', 'STOP', 'LONG', 
                  'SHORT', 'TOP', 'PICK', 'LOSS', 'INDEX', 'FUND', 'LINE', 
                  'LIVE', 'TECH', 'TIER', 'FREE', 'DATE', 'EDGE'}
    
    removed_count = 0
    kept_count = 0
    
    # For each unique ticker found, validate
    for ticker in sorted(set(tickers_found)):
        ...
        # Check 1: Is ticker in Knowledge Base?
        if ticker not in ticker_map:
            reason = f"not in KB (checked {ticker})"
        else:
            # Check 2: Do Thai variations appear in RAW?
            variations = ticker_map[ticker]
            
            # Also check ticker itself (sometimes used in speech)  
            all_variants = [ticker, ticker.lower()] + (variations if isinstance(variations, list) else [])
            
            found_variants = []
            for var in all_variants:
                if var and isinstance(var, str) and len(var) > 0 and var in raw_text:
                    found_variants.append(var)
                    is_valid = True
                    break  # Found at least one
            
            if is_valid:
                reason = f"✓ found '{found_variants[0]}' in RAW"
                kept_count += 1
            else:
                # Show which variations we checked
                sample_vars = variations[:3] if isinstance(variations, list) else []
                reason = f"KB has {sample_vars} but none in RAW"
    ...
    # Summary
    if removed_count > 0:
        print(f"   📊 Removed {removed_count} hallucinated ticker(s), kept {kept_count} valid")
    else:
        print(f"   ✅ All {kept_count} ticker(s) validated successfully")
    
    return corrected_text

def correction_node(state: AgentState):
    """
    Node: นักแก้ไข (Editor)

    ทำหน้าที่รับข้อความ (current_text) และข้อเสนอแนะ (feedback_msg) จากสถานะปัจจุบัน
    แล้วส่งให้ LLM (correction_chain) ทำการแก้ไขและเรียบเรียงข้อความใหม่ให้สละสลวยและถูกต้อง
    โดยใช้ Context จาก StockContextManager และ FinanceTermManager เพื่อเพิ่มความแม่นยำ
    ในการแก้ไขศัพท์เฉพาะทาง

    Args:
        state (AgentState): สถานะปัจจุบันของ Graph

    Returns:
        dict: อัปเดตสถานะด้วย 'current_text' ที่แก้ไขแล้ว และเพิ่ม 'iteration_count'
    """
    print(f"\n🔄 [Node] Correction (Round {state['iteration_count'] + 1})...")
    # ดึง Manager ที่เกี่ยวข้องจาก State มาใช้งาน
    ...
    if state['iteration_count'] == 0:  # Only on first iteration
        try:
            ...
            print(f"   ⚠️ Pre-processing in correction node failed: {e}")
    ...
    print("\n🧹 Deduplication...")
    print("   ⚠️  DEDUP DISABLED - Testing without deduplication")
    ...
    if False:  # Disabled for testing
        ...
        if manual_removed > 0:
            print(f"   🧹 Manual [DUP] markers: {manual_removed} sentences removed")
        ...
        if boundary_removed > 0:
            print(f"   🧹 Boundary dedup: {boundary_removed} duplicate(s) removed (threshold={thresholds['boundary']})")
        ...
        if num_removed > 0:
            print(f"   🧹 General dedup: {num_removed} duplicate(s) removed (threshold={thresholds['general']})")
            
    #except Exception as e:
    #    print(f"   ⚠️ Deduplication failed (continuing): {e}")
    
    # ==================================================================================
    # TICKER HALLUCINATION VALIDATION (Layer 2 - Code Validation)
    # ==================================================================================
    # TEMPORARILY DISABLED DUE TO CRITICAL SYSTEM FAILURE - 2025-12-29 15:19
    # 
    # Issues found:
    # 1. Validation removed ALL tickers (too strict)
    # 2. System produced corrupted output ("ดู ดู ดู..." spam)
    # 3. Need to fix text corruption first before re-enabling
    #
    # To re-enable: uncomment the code below after fixing bugs
    # ==================================================================================
    
    # print("\\n🛡️ Validating tickers (Layer 2 - Code Validation)...")
    # try:
    #     # Load KB data for validation
    #     kb_path = "knowledge_base.json"
    #     import json
    #     with open(kb_path, 'r', encoding='utf-8') as f:
    #         kb_data = json.load(f)
    #     
    #     # Validate and remove hallucinated tickers
    #     corrected = validate_ticker_hallucination(
    #         corrected_text=corrected,
    #         raw_text=state["raw_transcript"],
    #         kb_data=kb_data
    #     )
    #     print("   ✅ Ticker validation completed")
    # except Exception as e:
    #     print(f"   ⚠️  Validation error (continuing): {e}")
    
    # ล้างข้อเสนอแนะเก่าหลังจากใช้งานแล้ว เพื่อไม่ให้ส่งซ้ำในรอบถัดไป
    # SAFE DEDUPLICATION: Remove exact duplicates only
    try:
        from src.utils.safe_dedup import safe_deduplicate
        
        print("\n🧹 [Safe Dedup] Removing exact duplicates...")
        ...
        print(f"   ⚠️ Dedup error (skipping): {e}")
...
def fact_checking_node(state: AgentState):
    """
    Node: Fact Checker - ตรวจสอบความถูกต้องของข้อมูล
    
    ตรวจสอบ CLEAN output หลังจาก correction แล้วว่า:
    1. ไม่มีคำต้องห้าม (Fed, etc.)
    2. ราคามีชื่อหุ้นกำกับ (Price-Ticker Consistency)
    3. ราคาสอดคล้องกับตลาดจริงหรือไม่ (Market Validation)
   4. ไม่มี Hallucination
    
    ถ้าพบปัญหา จะเพิ่ม feedback เพื่อให้ correction_node แก้ไขในรอบถัดไป
    """
    print(f"\n🔍 [Node] Fact Checking...")
    ...
    try:
        ...
        if not result['is_valid']:
            ...
            for error in result['errors'][:3]:  # แสดง 3 รายการแรก
                print(f"      {error}")
        else:
        ...
        if result['warnings']:
            ...
            for warning in result['warnings'][:2]:  # แสดง 2 รายการแรก
                print(f"      {warning}")
        
        # แสดง market validation stats
        stats = result['statistics']
        if stats.get('recording_date'):
            print(f"   📅 Recording Date: {stats['recording_date']}")
            print(f"   📊 Market Validated: {stats.get('market_validated', 0)}/{stats.get('ticker_price_pairs', 0)} ticker-price pairs")
...
def auto_learning_node(state: AgentState):
    ...
    try:
        ...
        if corrections:
            print(f"   🔍 Found {len(corrections)} corrections to learn")
            ...
            if total_updates > 0:
                print(f"   ✅ Updated: KB={stats['kb_updates']}, Finance={stats['finance_updates']}, Errors={stats['error_updates']}")
        else:
            print(f"   ℹ️ No new patterns to learn from corrections")
        
        # 2. Log market validation issues (NEW)
        if validation_logs or validated_data:
            print(f"   📝 Logging {len(validation_logs)} validation issues")
            ...
            if error_entries:
                ...
                print(f"   ✅ Logged {len(error_entries)} errors to asr_errors.json")
            
    except Exception as e:
        print(f"   ⚠️ Auto-learning failed (continuing): {e}")
        import traceback
        traceback.print_exc()
...
def targeted_correction_node(state: AgentState):
    """
    Node: นักแก้ไขเฉพาะจุด (Targeted Editor)

    ทำงานคล้ายกับ correction_node แต่จะแก้ไขเฉพาะ Chunk ที่มีปัญหาเท่านั้น
    เพื่อลดเวลาและจำนวน API Request ใน Loop การแก้ไขรอบที่ 2 เป็นต้นไป
    """
    print(f"\n🎯 [Node] Targeted Correction (Round {state['iteration_count'] + 1})...")
    ctx_mgr = state["data_managers"]["stock_context"]
    term_mgr = state["data_managers"]["finance_term"]
    rate_limiter = state.get("rate_limiter") # Get RateLimiter
    
    # แปลง feedback เป็นข้อความ และเพิ่มลิสต์ชื่อหุ้นที่ต้องแก้ (ถ้ามี)
    ...
    feedback = state["feedback_msg"]
    if unknown_tickers:
        feedback += f" ตัวอย่างชื่อหุ้นที่ผิด: {unknown_tickers}"
    if state.get("correction_suggestions"):
        feedback += "\n" + "\n".join(state["correction_suggestions"])
    
    # Targeted Correction ต้องระบุ chunk indices ที่ต้องการแก้ใหม่เท่านั้น
    ...
    try:
        ...
        if manual_removed > 0:
            print(f"   🎯 Targeted Manual [DUP] markers: {manual_removed} sentences removed")
    except Exception as e:
        print(f"   ⚠️ Targeted manual dedup failed: {e}")
...
def fact_check_node(state: AgentState):
    """
    Node: นักตรวจสอบข้อเท็จจริง (Fact-Checker)

    ทำหน้าที่ค้นหาและตรวจสอบความถูกต้องของชื่อเฉพาะ (Proper Nouns) ที่ปรากฏในเนื้อหา
    เช่น ชื่อบุคคล, องค์กร, กองทุน, หรือศัพท์เทคนิคที่อาจถูก ASR ถอดเสียงมาผิด

    การทำงาน (จำลอง):
    1.  รับรายการ "คำที่ไม่รู้จัก" (Unknown Tickers) มาจาก Verification Node ของรอบก่อนหน้า
    2.  ตรวจสอบกับ Cache ถาวร (Persistent Cache) ใน Resolver ก่อนเป็นอันดับแรก
    3.  ถ้าเจอคำที่คล้ายกันใน Cache จะสร้างคำแนะนำการแก้ไขทันที เพื่อลดการเรียกใช้ API ภายนอก
    4.  หากไม่พบใน Cache จึงจะนำไปค้นหาจากแหล่งข้อมูลภายนอก (เช่น Google Search)
    5.  สร้างรายการคำแนะนำ (Correction Suggestions) เพื่อส่งให้ Correction Node ในรอบถัดไป

    Args:
        state (AgentState): สถานะปัจจุบันของ Graph

    Returns:
        dict: อัปเดตสถานะด้วย 'correction_suggestions' ที่ได้จากการตรวจสอบ
    """
    print("   🔍 [Node] Fact-Checking Proper Nouns (with Persistent Cache)...")
    suggestions = []
    unknowns = state.get("unknown_tickers", [])
    resolver = state["data_managers"]["resolver"] # ดึง Resolver ซึ่งมี Cache ถาวรอยู่ภายใน

    if not unknowns:
        return {"correction_suggestions": []}

    for unknown in unknowns:
        # 1. ตรวจสอบกับ Cache ถาวรใน Resolver ก่อน
        # ใช้ resolver.resolve() ซึ่งภายในมีการทำ Fuzzy Matching กับ Cache อยู่แล้ว
        cached_correction = resolver.resolve(unknown)
        
        if cached_correction:
            # ถ้าเจอใน Cache ให้สร้างคำแนะนำจาก Cache และข้ามไปทำคำถัดไป
            clean_ticker = cached_correction.replace(".BK", "")
            suggestions.append(f"- แนะนำให้แก้ไข: '{unknown}' อาจหมายถึง '{clean_ticker}' (อ้างอิงจาก Cache ที่เคยตรวจสอบแล้ว)")
            continue

        # 2. ส่วนนี้คือส่วนที่จะไป Search Google (หากไม่พบใน Cache)
        # สมมติว่าค้น Google แล้วเจอ
        if "macisen" in unknown.lower():
             suggestions.append("- แนะนำให้แก้ไข: 'Macisen Seven' อาจหมายถึง 'Magnificent Seven' (อ้างอิงจาก Web Search)")
...
def verification_node(state: AgentState):
    ...
    """
    print("   🔍 [Node] Sample Verification (FREE TIER)...")
    ...
    try:
        ...
        if chunks:
            ...
            print(f"   ⚡ Sampling chunk {sample_idx+1}/{len(chunks)} (saved {len(chunks)-1} requests)")
        else:
            sample_chunk = state["current_text"][:3000]
            print(f"   ⚡ Using first 3000 chars as sample")
        ...
        print(f"   ✅ Found {len(entities)} entities, Quality: {quality_score}/10")
    except Exception as e:
    ...
    valid_mappings = []
    unknowns_map = {} # ใช้ dict เพื่อเก็บว่าเจอ unknown ที่ chunk ไหน
    ...
    for ent in entities:
        ...
        if tk:
            ...
            # เพิ่ม/อัปเดต Cache ด้วยคำที่ถูกต้อง
            ...
            unknowns_map[ent.text_found] = 0 # สมมติว่าเจอที่ chunk 0
    
    unknowns = list(unknowns_map.keys())
    chunks_with_issues = list(set(unknowns_map.values()))
            
    if unknowns:
        feedback = f"คำเตือนจาก Auditor: พบชื่อหุ้นที่ไม่รู้จัก: {unknowns}. โปรดตรวจสอบและแก้ไข."
    elif quality_score < 8:
        feedback = f"คำแนะนำจาก Auditor: คุณภาพภาษายังไม่ดีพอ (คะแนน {quality_score}/10). เหตุผล: '{quality_reason}'. โปรดเรียบเรียงใหม่ให้สละสลวยขึ้น"
        if sample_chunk_index not in chunks_with_issues:
            chunks_with_issues.append(sample_chunk_index)
    else:
        feedback = "Auditor: คุณภาพถูกต้องแล้ว"
    ...
    return {
        ...
        "verified_entities_cache": current_cache # ส่ง cache ที่อัปเดตแล้วกลับเข้าระบบ
    }

def summary_node(state: AgentState):
    """
    Node: นักสรุป (Reporter/Summarizer)

    เป็น Node สุดท้ายในกรณีที่การแก้ไขเสร็จสมบูรณ์
    ทำหน้าที่รับ 'current_text' ที่ผ่านการแก้ไขทั้งหมดแล้ว
    และส่งให้ LLM เพื่อสร้างบทสรุปในรูปแบบ Markdown
    ...
    Returns:
        dict: อัปเดตสถานะด้วย 'final_summary'
    """
    print("\n📝 [Node] Generating Summary...")
    ...
    try:
        ...
        if len(lines) > 0:
            ...
            if most_common[1] > 10:  # Same line repeated 10+ times
                print(f"   ⚠️  WARNING: Detected {most_common[1]}x repetition!")
                ...
                print(f"   🔧 Applied aggressive dedup (removed {stats.get('removed_sentences', 0)} lines)")
        ...
        if is_quota_error:
            print("      ⚠️ Quota exceeded during summary generation.")
            print("      📝 Using fallback: Returning cleaned text as-is.")
            # Fallback: Return cleaned text formatted as basic markdown
            fallback_summary = f"# สรุปเนื้อหา - {state['channel_name']}\n\n"
            fallback_summary += "_⚠️ หมายเหตุ: ไม่สามารถสร้างบทสรุปได้เนื่องจาก API Quota หมด ข้อมูลด้านล่างเป็นข้อความที่ผ่านการแก้ไขแล้ว_\n\n"
            fallback_summary += "## เนื้อหาทั้งหมด\n\n"
            fallback_summary += state["current_text"]
            return {"final_summary": fallback_summary}
        else:
            print(f"      ❌ Unexpected error during summary: {e}")
            # Generic fallback
            return {"final_summary": f"# Error\n\nไม่สามารถสร้างบทสรุปได้: {e}"}
...
def price_validation_node(state: AgentState):
    ...
    - NO auto-correction
    """
    print("\n⚠️  [Price Validation] Checking prices against market data...")
    
    summary_text = state.get("final_summary", "")
    
    if not summary_text or len(summary_text) < 100:
        print("   ℹ️  Summary too short, skipping validation")
    ...
    try:
        ...
        stats = result['stats']
        print(f"   📊 Checked: {stats['checked']} prices, Warnings: {stats['warnings']}, Errors: {stats['errors']}")
        
        if not result['has_warnings']:
            print("   ✅ All prices within reasonable range (52-week ±10%)")
        ...
        # Display warnings
        print(f"\n   ⚠️  {len(result['warnings'])} price warnings detected:")
        for warning in result['warnings'][:3]:  # Show first 3
            print(f"      • {warning['ticker']}: {warning['message']}")
    ...
    ตรวจสอบ SUMMARY ด้วย yfinance และแก้ไขราคาที่ผิดพลาดก่อนส่ง output
    
    Workflow:
    1. ตรวจสอบราคาใน summary กับ market data (15% tolerance)
    2. ถ้าผิด → สร้าง hints (phonetic + market constraints)
    3. ขอให้ LLM สร้าง summary ใหม่ด้วย hints
    4. ออก final summary ที่ถูกต้อง
    """
    print("\n💹 [Post-Summary] Market Validation & Correction...")
    
    summary_text = state.get("final_summary", "")
    
    if not summary_text or len(summary_text) < 100:
        print("   ⚠️ Summary too short, skipping market validation")
        return {}
    ...
    try:
        ...
        if video_title:
            # Parse date from title (e.g., "LIB Tech Talks วันที่ 03 ธ.ค. 2568")
            from src.validation.fact_checker import CleanFactChecker
            checker = CleanFactChecker()
        ...
        if not video_date:
            ...
            print(f"   ⚠️ Could not detect video date, using today: {video_date}")
        else:
            print(f"   📅 Using video date: {video_date}")
        
        validator = MarketPriceValidator()
        result = validator.validate_prices(summary_text, date=video_date)
        
        print(f"   📊 Found {result['total_found']} prices, Validated: {result['validated']}/{result['total_found']}")
        
        # ถ้าทุกอย่างถูกต้อง → ผ่าน
        if not result['errors']:
            print(f"   ✅ All prices validated (within 15% tolerance)")
            return {}
        
        # มีราคาผิดพลาด → สร้าง correction hints
        print(f"   ⚠️ {len(result['errors'])} price mismatches detected:")
        for error in result['errors'][:3]:  # Show first 3
            print(f"      {error}")
        
        # สร้าง hint message สำหรับ LLM (แบบเงียบ - ไม่ต้องบอก)
        hint_message = "\n\n**PRICE CORRECTION REQUIRED:**\n\n"
        
        for error in result['errors']:
            # Parse error to extract prices
            import re
            match = re.search(r'([A-Z]+).*?(\d+\.?\d*) บาท.*?(\d+\.?\d*) บาท', error)
            if match:
                ticker = match.group(1)
                wrong_price = match.group(2)
                market_price = match.group(3)
                hint_message += f"- {ticker}: แก้จาก {wrong_price} บาท → เป็น {market_price} บาท\n"
        
        hint_message += "\n**IMPORTANT:**\n"
        hint_message += "1. แก้ไขราคาให้ตรงกับตลาด (tolerance 10-15% สำหรับ ASR error)\n"
        hint_message += "2. แก้เงียบ ๆ ไม่ต้องเขียน '(Correction:...)' หรือ '(Note:...)' \n"
        hint_message += "3. summary ควรสะอาด อ่านง่าย ไม่มี alert text\n"
        
        print(f"\n   🔄 Re-generating summary with market hints...")
        ...
        print(f"   ✅ Market-corrected summary generated (cleaned)")
        
        return {"final_summary": final_corrected}
        
    except Exception as e:
        print(f"   ⚠️ Market correction error (skipping): {e}")
...
def router(state: AgentState):
    ...
    if has_unknown and state["iteration_count"] >= 2:
        print(f"   ⚠️ Accepting {len(state['unknown_tickers'])} unknown entities after 2 tries")
        has_unknown = False
...
def build_workflow(data_managers: Dict[str, Any]):
    """
    สร้างและคอมไพล์ Workflow ของ LangGraph

    ประกอบด้วยการกำหนด Node และ Edge ทั้งหมด:
    - Entry Point: 'correct'
    - 'correct' -> 'auto_learning' (เรียนรู้จากการแก้ไข)
    - 'auto_learning' -> 'fact_check' (ตรวจสอบข้อเท็จจริง)
    - 'fact_check' -> 'verify' (ตรวจสอบ Ticker และคุณภาพ) 
    - 'verify' -> 'router' -> (วนกลับไป 'correct'/'targeted_correct' หรือไป 'summarize') 
    - 'summarize' -> END

    Args:
        data_managers (Dict[str, Any]): Dictionary ที่เก็บ instance ของ Manager ต่างๆ

    Returns:
        CompiledGraph: แอปพลิเคชัน LangGraph ที่พร้อมใช้งาน
    """
    ...
    app.config = {"recursion_limit": 50}  # เพิ่มจาก default 25 เป็น 50
